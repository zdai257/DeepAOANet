...
parser.add_argument('--theta1', type=str, required=False, help='Specify thata of impinging signal 1')
parser.add_argument('--theta2', type=str, required=False, help='Specify thata of impinging signal 2')
...
print("Delta_carrier_phase = %.3f (rad)\n" % delta_phase)
...
for idx_sigma, (sigma_name, noise_sigma) in enumerate(gauss_sigma_dict.items()):
    ...
    for idx, (key, angle_val) in enumerate(angle_dict.items()):
        ...
        theta1_name = str(int(theta1) + 360)
        theta2_name = str(int(theta2) + 360)

        #pub = rospy.Publisher('/kerberos/R_' + theta1_name + '_' + theta2_name, Float32MultiArray, queue_size=100)
        pub = rospy.Publisher('/kerberos/R_multi', Float32MultiArray, queue_size=100)
        tmp_dict[idx] = pub
...
sync_client = message_filters.ApproximateTimeSynchronizer([sub1, sub2], queue_size=1, slop=.2, allow_headerless=True)

    ...
    def callback(self, msg):
        ...
        if iq_start > self.IQamp_thres and iq_end > self.IQamp_thres:
            ...
            for win_idx, win_val in enumerate(self.win_lst):
                ...
                data_arr = np.append(new_R_real.reshape((self.M, self.M, 1)), new_R_imag.reshape((self.M, self.M, 1)),
                                     axis=2)
...
if __name__ == "__main__":
    ...
    p = win.addPlot(title="Real-Time AoA Polar Power Spectrum")  # creates empty space for the plot in the window
    ...
    radius = np.random.normal(loc=0, scale=0.05, size=36)
    ...
    # Set X Axis
    p.setLabel('bottom', "Cos(THETA) projection")
    p.setLabel('left', "Sin(THETA) projection")
